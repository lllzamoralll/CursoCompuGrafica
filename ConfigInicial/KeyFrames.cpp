...
// Grupo 7
// Practica extra - skybox
// fecha de entrega: 09/11/25
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "practica extra, Sanchez Zamora Jesus", nullptr, nullptr);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// Primero rotar la inclinaci�n hacia atr�s (eje X)
...
void DoMovement()
{
	...
	if (keys[GLFW_KEY_0])  // Inclinar cuerpo hacia atr�s
	{
		bodyIncline -= 0.25f;
		if (bodyIncline >= 60.0f) // M�ximo 60 grados
			bodyIncline = 60.0f;
	}

	if (keys[GLFW_KEY_9])  // Inclinar cuerpo hacia adelante
	{
		bodyIncline += 0.25f;
		if (bodyIncline <= 0.0f) // No menos de 0 grados
			bodyIncline = 0.0f;
	}
