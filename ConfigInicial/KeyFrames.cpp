...
int main()
{
	...
	{
		// === ANIMACIÓN CORREGIDA: Sentarse suave + Saludar con pata derecha (SUBE) ===
		...
		KeyFrame[1].bodyIncline = 15.0f;  // Suave

		// Keyframe 2: Sentado completo (máx 28°)
		...
		KeyFrame[2].bodyIncline = 28.0f;  // ¡NO MÁS!
		...
		FrameIndex = 5;
		printf("Animación FINAL: sentado suave + pata derecha SUBE al saludar\n");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// Primero rotar la inclinación hacia atrás (eje X)
		model = glm::rotate(model, glm::radians(-bodyIncline), glm::vec3(1.0f, 0.0f, 0.0f));
		// Luego rotar sobre Y como siempre
		...
		// Front Right Leg (recta, sube en saludo)
		model = modelTemp;
...
void DoMovement()
{
	...
	// Traseras (no mucho hacia adelante)
	if (keys[GLFW_KEY_U])
	{
		bLeftLeg += 0.25f;
		bRightLeg -= 0.25f;
		if (bLeftLeg >= 60.0f && bRightLeg <= -60.0f) {
			bLeftLeg = 60.0f;
			bRightLeg = -60.0f;
		}
	}
	...
	// Delanteras (rectas, no se levantan)
	if (keys[GLFW_KEY_C])
	{
		fLeftLeg += 0.25f;
	...
	{
		fRightLeg += 0.25f;
		if (fRightLeg > 90.0f) fRightLeg = 90.0f;  // Sube solo para saludo
	}
	if (keys[GLFW_KEY_X])
	{
		fRightLeg -= 0.25f;
