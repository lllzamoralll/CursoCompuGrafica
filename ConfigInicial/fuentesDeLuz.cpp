...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Fuentes de luz, Sanchez Zamora Jesus", nullptr, nullptr);
    if (nullptr == window)
    {
    ...
    Model Dog((char*)"Models/plantita/plantita.obj");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        model = glm::mat4(1);
        // Activar blending y dibujar la bola con transparencia
        glEnable(GL_BLEND); // Activa el canal alpha
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA); // Configura la función de blending
        model = glm::translate(model, glm::vec3(0.0f, 0.0f, 0.0f)); // Posición sobre el piso
        model = glm::scale(model, glm::vec3(3.0f)); // Escala la bola
        glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
        glUniform1i(glGetUniformLocation(lightingShader.Program, "transparency"), 1); // Indica transparencia
        Dog.Draw(lightingShader); // Dibuja ball.obj con transparencia
